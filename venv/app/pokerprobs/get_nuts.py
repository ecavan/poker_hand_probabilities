...
def deal_new(deck, num_player,num_player_in_hand, remove_cards):

    deck.remove(remove_cards[0])
    deck.remove(remove_cards[1])
    deck.remove(remove_cards[2])
    deck.remove(remove_cards[3])
    deck.remove(remove_cards[4])
    ...
    cards = rng.choice(deck, size=2*(num_player-1) + 4, replace=False)
    community_cards = cards[2*(num_player-1):]
    
    turn = community_cards[1]
    river = community_cards[3]
    ...
    new_players = []
    new_players2 = []
    for i in range(num_player_in_hand):
        new_players.append(players[player_list[i]])
        temp = [(int(new_players[i][0][0]), new_players[i][0][1]), (int(new_players[i][1][0]), new_players[i][1][1])]
        new_players2.append(temp)


    return new_players2, turn, river
...
def get_nuts(num_player,num_players_in_hand, hand,flop, num_sims = 3000):

    card1 = hand[0]
    card2 = hand[1]
    card3, card4, card5 = flop[0], flop[1], flop[2]
    
    remove_cards = []
    
    for i in [card1, card2, card3, card4, card5]:
...
#         card2 = (int(card2[0]), card2[1])
    ...
    for i in range(num_sims):
        ...
        players, turn, river = deal_new(deck, num_player,num_players_in_hand, remove_cards)
...
#         players2_no_fold = players2[0:num_players_in_hand]
        ...
        op_hands = new_players
        for j in range(len(op_hands)):
            results = get_result_nuts(op_hands[j][0],op_hands[j][1],cards2[0],cards2[1],cards2[2],cards2[3],cards2[4])
